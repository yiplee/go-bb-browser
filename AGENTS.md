...
- **Running end-to-end requires a real Chrome — the daemon never launches it.** Chrome is preinstalled (`google-chrome`). There is no display, so launch **headless**: `./bb-browser launch --headless --port 9222` (prints `host:port`), then `./bb-daemon --debugger-url 127.0.0.1:9222 --listen 127.0.0.1:8787`. Verify with `curl 127.0.0.1:8787/health`. The CLI defaults to `http://127.0.0.1:8787`.
